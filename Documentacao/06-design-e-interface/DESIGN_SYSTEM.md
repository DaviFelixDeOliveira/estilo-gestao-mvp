...
\- possuir estado fechado;

\- possuir estado aberto;

\- indicar resposta em processamento;

\- permitir erro;

\- permitir nova tentativa;

\- manter campo acessível.

\---

**# 141. Estados do Assistente IA**
...
O estado de Loading não deverá parecer uma mensagem enviada pelo próprio assistente.

\---

**# 142. Limite e timeout da IA**
...
Apresentar estados distintos para:

\- proteção temporária contra abuso;

\- encerramento da conversa ao atingir 20 mensagens;

\- aviso ao atingir 80% das 1.000 respostas do ciclo;

\- bloqueio da IA ao atingir 100% da cota mensal.

Não exibir nem sugerir histórico de conversas, pois o conteúdo não é guardado no banco.

\---

**# 143. Stitch**

O Stitch será utilizado para:
...
O protótipo não substitui a documentação funcional.

Se o Stitch inventar:
...
isso não passa automaticamente a fazer parte do sistema.

\---

**# 144. Consistência entre telas**

Uma decisão visual aprovada deverá ser reutilizada.
...
Não redesenhar os mesmos componentes a cada nova tela.

\---

**# 145. Componentes mínimos**
...
\- Product Image / Product Image State;
...
\- Admin Business Row/Card.

Não é obrigatório que cada item exista como arquivo de componente separado se não houver benefício técnico.

\---

**# 146. Estados obrigatórios no protótipo**

Telas importantes deverão representar, quando aplicável:
...
\- Tema Escuro quando a tela já estiver sendo validada visualmente.
...
**# 147. Critérios de validação de componentes**

Antes de considerar um componente visualmente aprovado, verificar se:

\- possui estado padrão;

\- possui hover quando aplicável;
...
\- possui estado desabilitado;

\- possui erro quando aplicável;

\- possui nome acessível;

\- funciona por toque;

\- não depende somente de cor;

\- não depende somente de hover;

\- utiliza tokens do Design System;

\- funciona no Tema Claro;

\- funciona no Tema Escuro quando aplicável;

\- mantém consistência com componentes equivalentes;

\- foi verificado em Mobile;

\- foi verificado em Desktop.

\---

**# 148. Critérios de validação de formulários**

Antes de considerar um formulário aprovado, verificar se:

\- labels estão visíveis;

\- campos obrigatórios estão identificados;

\- campos opcionais estão claros quando necessário;

\- validações aparecem próximas aos campos;

\- dados são preservados em erros recuperáveis;

\- *\`Enter\`* funciona quando apropriado;

\- *\`textarea\`* mantém *\`Enter\`* como quebra de linha;

\- Loading impede múltiplos envios;

\- botões possuem estado desabilitado;

\- primeiro erro recebe atenção adequada;

\- senha possui Mostrar/Ocultar;

\- autocomplete apropriado foi considerado;

\- teclado Mobile adequado foi considerado;

\- ação principal permanece acessível;

\- erros do servidor são apresentados com segurança;

\- sucesso fornece feedback;

\- formulário funciona no Tema Claro;

\- formulário funciona no Tema Escuro.

\---

**# 149. Critérios de validação do Tema Escuro**

Antes de considerar o Tema Escuro aprovado, verificar:
...
Nenhum componente deverá simplesmente desaparecer porque alguém decidiu pintar texto grafite sobre fundo grafite. Um clássico da engenharia cromática humana.

\---

**# 150. Critérios de validação da área ADMIN**

Antes de considerar o Painel Administrativo aprovado, verificar se:

\- a área está claramente identificada como administrativa;

\- utiliza o mesmo Design System;

\- possui navegação própria;

\- não possui funções desnecessárias da barbearia;

\- não exibe informações privadas além das necessárias;

\- ações de impacto possuem confirmação;

\- busca de barbearias possui Empty State;

\- estados de Loading e erro estão definidos;

\- funciona em Web e Mobile;

\- funciona nos temas suportados.

\---

**# 151. Critério de conclusão**

O Design System estará suficientemente definido quando:

\- cores principais estiverem aplicadas consistentemente;

\- Tema Claro estiver representado;

\- Tema Escuro estiver representado;

\- Padrão do Sistema estiver definido;

\- componentes utilizarem comportamento compatível com temas;

\- componentes principais estiverem padronizados;

\- formulários utilizarem o mesmo comportamento;

\- Loading, erro, vazio e sucesso estiverem previstos;

\- navegação por teclado estiver funcional;

\- ações por toque forem confortáveis;

\- OTP possuir comportamento completo;

\- ações destrutivas possuírem confirmação;

\- interfaces não dependerem exclusivamente de hover;

\- *\`BARBEIRO\`* e *\`ADMIN\`* utilizarem contextos de interface adequados;

\- não existir seletor público para criação de ADMIN;

\- despesas recorrentes possuírem estados consistentes;

\- uploads de imagem possuírem preview, Loading, erro, substituir e remover;

\- produtos possuírem estados visuais consistentes para imagem personalizada, imagem padrão da categoria e ausência de imagem;

\- remoção de imagem personalizada permitir retorno ao padrão da categoria ou estado sem imagem quando aplicável;

\- imagens padrão do sistema não serem apresentadas como arquivos editáveis pelo barbeiro;

\- Product Cards manterem layout consistente independentemente da origem da imagem;

\- navegação principal do BARBEIRO possuir apenas Dashboard, PDV, Operação e Configurações;

\- sidebar Desktop possuir estados expandido e recolhido;

\- bottom navigation Mobile possuir os quatro módulos principais;

\- Operação agrupar visualmente Serviços, Produtos e Estoque;

\- Configurações agrupar Negócio e Barbearia;

\- item principal e submenu ativos serem visualmente identificáveis;

\- menu rápido da conta não expor diretamente Excluir conta;

\- Zona de perigo permanecer visualmente separada;

\- navegação funcionar em Tema Claro e Tema Escuro;

\- Vitrine possuir estados de criação, publicação e erro;

\- Perfil e Conta estiverem visualmente separados das Configurações da Barbearia;

\- relatórios possuírem ações claras de PDF e PNG;

\- protótipos no Stitch seguirem estes padrões;

\- implementação frontend reutilizar componentes em vez de recriar estilos isolados por tela.
...
Para a exceção administrativa, exigir justificativa e o código na frase *\`EXCLUIR EG-XXXXXX\`*.
